...
\begin{tikzpicture}[
declare function={ kpsi(\x) =0.5*\B*\L*cos(\x)-0.5*\B*\L;
myfunc(\x)=sin(\x) * sin( 180/pi*kpsi(\x))/kpsi(\x);
                 },
]
\begin{polaraxis}[hide axis]
%aperture
\addplot[domain=0.5:360,samples=720] {myfunc(x)}; 
\end{polaraxis}
\end{tikzpicture}%
\end{urdufont}
\end{document}
