...
%axis
\draw[thick] (-\x,0)--(\x+1,0)node[right]{$x$};
...
\draw  ({(\ang+90)/2}:1.2) node[gray,rotate=-\ang]{\small \RL{زاویہ آمد}};
\draw[stealth-stealth] ([shift={(90:0.8)}]0,0) arc (90:180-\angR:0.8);
\draw  (140-\angR:1.1) node{$\theta_r$};
\draw  ({(270-\angR)/2}:1.5) node[gray,rotate={(90-\angR)/2}]{\RL{زاویہ انعکاس}};
%transmission
\draw[-latex] (0,0)--++(-90-\angT:2.8)coordinate[pos=0.7](kT)node[gray,shift={(90:0.3)},left]{\RL{ترسیلی موج}};
\draw[stealth-stealth] ([shift={(-90:0.5)}]0,0) arc (-90:-90-\angT:0.5);
\draw (-90-\angT/2:0.8) node{$\theta_t$};
\draw (-90-\angT/2:1.6) node[align=right,gray]{زاویہ \\ انحراف};
...
%medium
\draw (\x+0.5,0.3) node[left]{\RL{{\small خطہ-1:} ${\scriptstyle \epsilon_1,\mu_1,\sigma_1,Z_1,\eta_1}$}};
\draw (\x+0.5,-0.3) node[left]{\RL{{\small خطہ-2:} ${\scriptstyle \epsilon_2,\mu_2,\sigma_2,Z_2,\eta_2}$}};
%
\end{tikzpicture}%
\end{urdufont}
\end{document} 
