\documentclass[varwidth,border=5pt]{standalone}

\usepackage{tikz}
\usepackage{tikz-3dplot}
\usetikzlibrary{calc}
\usepackage[active,tightpage]{preview}
\PreviewEnvironment{tikzpicture}

%========
\begin{document}
%\def\boundb{(-2,2) rectangle (4,-2)}

\tdplotsetmaincoords{70}{110}
\begin{tikzpicture}[scale=3,tdplot_main_coords]
\coordinate (O) at (0,0,0);
\coordinate (N1) at (1,0.5,0);
\coordinate (N2) at (0,0.5,1);
\coordinate (N) at (1,0,0.6);

    	\draw[thick,->] (0,0,0) -- (1,0,0) node[anchor=north east]{$x$};
	\draw[thick,->] (0,0,0) -- (0,1,0) node[anchor=north west]{$y$};
    	\draw[thick,->] (0,0,0) -- (0,0,1) node[anchor=south]{$z$};
%
\draw[gray,-latex](O)--(N1)node[pos=0.4, right]{$\bf{r_1}$} node[below,black]{$N_1$};
\draw[gray,-latex](O)--(N2)node[pos=0.4,right]{$\bf{r_2}$} node[above,black]{$N_2$};
\draw[gray,-latex](O)--(N)node[pos=0.4,above]{$\bf{r_N}$} node[shift={(180:0.4cm)},black]{$N_3$};
%
\draw[thin, gray,-latex](N1)--(N);
\draw[shorten <=2.3cm,shorten >=-.5cm,-latex](N1) --(N);
\draw[thin, gray,-latex](N2)--(N);
\draw[shorten <=3.1cm,shorten >=-.5cm,-latex](N2)--(N);

%\draw[blue](current bounding box.south west) rectangle (current bounding box.north east);
%\draw[help lines, step=10pt](0,0)grid(1,1);
%\path[use as bounding box]( -1.2,-1.5,-1)rectangle(1,2,2);
\end{tikzpicture}

\end{document}
