...
\pgfmathsetmacro{\xLen}{-1.15}

\begin{document}
\begin{urdufont}
\begin{tikzpicture}
%shorted line wave
\draw[dashed] ({7.5*0/360},{waveWithReflection(-1,0,0)}) \foreach \x in {0,-1,...,-360}{--({7.5*\x/360},{waveWithReflection(-1,0,\x)})};
%axis
\draw (-1,0)--++(0,2);
\draw (-1,0)--(7.3,0);
\draw[stealth-] (7.15,0)++(0,-0.1)--++(0,-0.5)node[below]{$\SI{-11.5}{\centi\meter}$};
\draw[dashed](7.15,0)--++(0,2);
\foreach \y in {1,2,3}{\draw[gray,dashed] (-1,0.55*\y)node[left]{$\y$}--++(8.3,0);}
\draw[gray] (-1.7,2*0.5) node[rotate=90]{\RL{اوسط برقی دباو}};
\draw[] (1.9,-0.7)node{\RL{فیتہ}};
%min marks
\draw[stealth-] (7.5*180/360,0)++(0,-0.1)--++(0,-0.5)node[below]{$\SI{26}{\centi\meter}$};
\foreach \x/\lab in {0/0,1/10,2/20,3/30,4/40,5/50,6/60}{\draw ({6.35-\x},0)--++(0,-0.1) node[below]{$\lab$};};
%loaded line's standing wave
\draw[] ({7.5*0/360},{waveWithReflection(0.42484,21.557,0)}) \foreach \x in {0,-1,...,-360}{--({7.5*\x/360},{waveWithReflection(0.42484,21.557,\x)})};
%text
\draw(-7.5*270/360,1.9)node[above]{\RL{کسر دور}};
\draw(-7.5*170/360,1.3)node[above]{\RL{بار بردار}};
%axis
\draw (\xSH,0)--++(-7.5,0)coordinate(originL)--++(0,2.2);
