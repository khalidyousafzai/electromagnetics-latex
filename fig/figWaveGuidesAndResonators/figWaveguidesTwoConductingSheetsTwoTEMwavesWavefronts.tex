...
%wave lengths
\draw[gray] (startAa)++(\angU:0.05)--++(\angU:0.8)coordinate[pos=0.9](klamTop);
\draw[gray,stealth-stealth] (klamTop)--++(\angD:\wL)node[pos=0.45,fill=white,scale=0.5]{$\frac{\lambda_0}{2}$};
\draw[gray,stealth-stealth] (startLambda)--(endLambda)node[pos=0.5,fill=white,scale=0.5]{$\frac{\lambda}{2}$};
% wave guide
\draw[] (\angU:1*\wL)--++(\l,0);
\draw[] (\angD:1*\wL)--++(\l,0);
\end{tikzpicture}%
