...
\pgfmathsetmacro{\ang}{90}   %angle going upwards
...
%guide
\draw (-0.5,0)--(\x,0)coordinate(xR);
\draw (-0.5,\y)node[above right]{$\lambda_0=\lambda_{0c}$}--(\x,\y)node[above left]{$\theta=90^{\circ}$};
%
\draw[latex-latex] (0,0)coordinate(ruB)--++(\ang:\l)coordinate(ruT)coordinate[pos=0.85](ruM);
